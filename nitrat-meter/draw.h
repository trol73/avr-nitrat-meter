...
//Вывод верхнего заголовка
  ...
  lcd_OutChar(167);                           //Символ пустого треугольника влево <|
  LcdNadp_10(3);                         //" ПРОДУКТ: "
//Вывод курсора
  lcd_GotoXY(0, 1+n_prod-sdvig_prod); //x=0, y=1...5
  lcd_OutChar(166);                           //Символ закрашенного треугольника вправо |>
...
//Вывод стрелок вверх-вниз в последней колонке дисплея
  lcd_GotoXY(13, 1);
  if (sdvig_prod>0) lcd_OutChar(163);         //Символ пустого треугольника вверх ^
  lcd_GotoXY(13, 5);
  if (sdvig_prod<31) lcd_OutChar(161);        //Символ пустого треугольника вниз V
//
...
//Вывод стрелки-указателя влево
  lcd_GotoXY(0, 0);                   //x=0, y=0
  lcd_OutChar(167);                           //Вывод символа пустого треугольника влево <|
  ...
  { if (Gmax!=Gmin)                      //Если Gmax=Gmin - проценты не выводятся
    { if (Gx>=Gmin)                      //Если % положительные
      { datl=Gx;
        ...
        if (dat<999) lcd_OutIntBuf(2, dat, 3, 0, 0);//Если <999 - запись в текстовый буфер вычисленных %
      ...
      { datl=Gmin;                       //Изменение типа переменной на unsigned long
        ...
        if (dat<99) lcd_OutIntBuf(1, dat, 2, 0, 0);//Если <99 - запись в текстовый буфер вычисленных %
        else sprintf(lcd_buf, "??");     //Иначе - копирование в буфер строки "???"
...
*/
  ...
  //Вывод надписей первой строки меню
    ...
    { if (Gx==65535) sprintf(lcd_buf, "????.?");//Если признак переполнения - копирование строки в буфер
      else lcd_OutIntBuf(3, Gx, 4, 1, 0);   //Иначе - вывод в буфер проводимости в мкСм
  ...
  //Вывод надписей второй строки меню
    ...
    lcd_OutCharXY(d, 0, 2);                   //Вывод символа в позицию x=0, y=2
