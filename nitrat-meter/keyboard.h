...
{ if (knopka() > 2)                        //Если нажата любая кнопка, кроме "Вкл./Выкл."
	...
	{ b=0;                                 //Установка признака первого нажатия кнопки
		...
		{ if (menu_cnt<2)                    //Если ещё не меню "Замер"
			...
			{ if (meassureScreen==0)                  //Если пункт "Замер"
				{ TCCR0=0b01101001;              //Пуск т.сч.0 тестового сигнала (FastPWM, OC2=0 при совпад.,OC2=1 при TOP=255,F=2МГц/1/256=7812,5Гц
					...
					if (!meassureOk)               //Если замер неудачный
					...
					T_menu=1;                      //1 - признак прошедшего интервала 0,26 сек для вывода меню
					meassureStatus=1;                         //Признак выполненного, но не сохраненного замера
					TCCR0=0b01101000;              //Стоп т.сч.0 тестового сигнала
				}
				else
				{ if (meassureScreen==1)                //Если пункт "Сохранить"
					{ if ((meassureStatus==1) && meassureOk) //Если не сохраненный и корректный замер
						{ saveMeasure();             //Сохранение результатов замера продукта в EEPROM
							meassureStatus=2;                     //Признак сохраненного замера
							meassureScreen=0;                 //Перемещение курсора на пункт замер
					...
					{ if (meassureScreen==3)              //Если пункт "Очистить статистику"
						{ if (akt==0)
		...
		{ if (mnc_nach==1)                   //Иначе, если выбран пункт "Настройки"
			{ if (menu_cnt==3)                 //Если меню "Настройки"
				{ if (akt==1)                    //Если пункт меню "Настройки" активен
					...
					{ if (settingsScreen<5)             //Иначе, если не меню "Калибровка"
		...
		{ if (menu_cnt==0)                   //Если начальное меню
			...
			{ if (menu_cnt==1)                 //Если меню выбора продукта
				{ if (n_prod<35)                 //Если счетчик продукта <35
					{ n_prod++;
						if ((n_prod<30)&&(meassureScreen==4)) meassureScreen=3;//Если название продукта из flash и пункт Ред.назв.->Очистка статист.
						if ((n_prod-sdvig_prod)>4)
						{ sdvig_prod++;
						}
						meassureStatus=0;                       //Признак ещё не проведенного замера выбранного продукта
						if (meassureScreen==1) meassureScreen=0;   //Если был пункт "Сохранить" - изменение на пункт "Замер"
						bip=4;                       //1 короткий звук 0,1 сек, 3.5 кГц
					}
				}
				else
					...
					{ if (menu_cnt==3)             //Если меню "Настройки"  mnc_nastr: 0-время подсв.,1-яркость,2-таймер,3-звук,4-контрастность,5-Uакк,6-Rx
						{ if (akt==0)                //Если выбранный пункт не активен
							{ if ((settingsScreen!=4)&&(settingsScreen<6))//Если не пункт Контрастность и еще не пункт Rx
								...
								{ if (settingsScreen==4)      //Если пункт Контрастность - переход с задержкой в меню Калибровка
									{ b=0;
						...
						{ if (akt==0)                //Если выбранный пункт не активен
							...
							{ if (meassureScreen==3)          //Если пункт "Очистить статистику"
								{ eeprom_write_word(mas_Nzam+n_prod, 0);//Запись 0 замеров в EEPROM
									...
									T_menu=1;              //Установка признака прошедшего интервала 0,26 сек для вывода меню
									if (meassureStatus==2) meassureStatus=1;     //Если был сохраненный замер - признак не сохраненного замера
...
unsigned char autopovt(uint8_t k, uint8_t t) {
	if (knopka() != k) {                       //Если кнопка все еще нажата
		return 1;			// если кнопка не нажата - выход с "1"
	}
	b++;
	if (b == 1) {                            //Если первое нажатие
		while (b < 15) {
			_delay_ms(60);
			if (knopka() != k) 
				return 1;      //Если кнопка отпущена в течение
			b++;                             //0,9 сек - выход с "1"
		}
