...
void opros() {
	...
	if (knopka() > 2) {                       //Если нажата любая кнопка, кроме "Вкл./Выкл."
		...
		if (!light) {                        //Если подсветка была выключена по таймеру
			light = true;                           //Установка признака включенной подсветки
		...
		if (rezh == 0) {                        //Если был признак выключения
			...
			while (knopka()) {               //Пока нажата заданная кнопка
				batt();                          //Измерение напряжения аккумулятора и управление зарядкой
				DrawMenu();                          //Вывод меню каждые 0.26 сек
			}
		}
	} else {
		return;
	...
	uint8_t t = 100;                   //Период автоповтора нажатой кнопки в мс/2
	uint8_t bip = 255;                 //Переменная вида звукового сопровождения (255-нет звука)
	register uint8_t nkn = knopka();   //Регистровая переменная для кода нажатой кнопки
	...
	if (nkn == 6) {                           //Если нажата кнопка ВПРАВО
		...
		if (mainMenuIndex == MAIN_MENU_MEASSURE) {
			if (menu_cnt < 2) {                    //Если ещё не меню "Замер"
				...
				switch (meassureScreen) {
					case MEASSURE_SCREEN_MEASSURE:
						...
						T_menu = 1;                      //1 - признак прошедшего интервала 0,26 сек для вывода меню
						meassureStatus=3;                         //Признак непрерывного (не сохраненного) замера
						DrawMenu();                        //Вывод меню, там же вызывается вычисление Gx, Rx и meassureOk
						beep(meassureOk ? 7 : 6);
						...
						T_menu=1;                      //1 - признак прошедшего интервала 0,26 сек для вывода меню
						meassureStatus = MEASSURE_STATUS_NOT_SAVED;                         //Признак выполненного, но не сохраненного замера
						TCCR0 = 0b01101000;              //Стоп т.сч.0 тестового сигнала
						break;

					case MEASSURE_SCREEN_SAVE:
						if (meassureStatus == MEASSURE_STATUS_NOT_SAVED && meassureOk) { //Если не сохраненный и корректный замер
							saveMeasure();             //Сохранение результатов замера продукта в EEPROM
								meassureStatus=2;                     //Признак сохраненного замера
								meassureScreen = MEASSURE_SCREEN_MEASSURE;                 //Перемещение курсора на пункт замер
								drawMessage(MSG_MEASSURE_STORED);               //Вывод сообщения "Замер сохранён!"
								beep(1);                  //1 короткий звук 0,1 сек, 3 кГц
					...
					case MEASSURE_SCREEN_CLEAR:
						akt = !akt;
						bip=4;                     //1 короткий звук 0,1 сек, 3,5 кГц
						break;

					case MEASSURE_SCREEN_EDIT_NAME:
						if (!akt) {              //Если пункт не активирован
							akt=1;                 //Активирование
						...
						} else if (n_nazv < 8) {                   //Иначе, если пункт активирован
							while (knopka() == 6) {  //Пока нажата кнопка ВПРАВО
								if (n_nazv < 7) {
									n_nazv++;        //Инкремент счетчика букв пользовательского названия продукта
									bip = 4;           //1 короткий звук 0,1 сек, 3,5 кГц
								} else {
									bip = 5;        //1 короткий звук 0,1 сек, 2,5 кГц
								}
								T_menu=1;          //1 - признак прошедшего интервала 0,26 сек для вывода меню
								batt();            //Измерение напряжения аккумулятора каждые 0.26 сек
								DrawMenu();            //Вывод меню
								if (b == 0) {
									beep(bip);//Если первое нажатие кнопки в цикле автоповтора - звук
								}
								bip = 255;           //Признак отсутствия звука
								if (autopovt(6, 150) == 1) {
									break;//Если при автоповторе кнопка отпущена - выход
								}
							}
						}
						break;
		...
		} else if (mainMenuIndex == MAIN_MENU_SETTINGS) {                   //Иначе, если выбран пункт "Настройки"
		...
		} else if (mainMenuIndex == MAIN_MENU_LIMITS) {
			if (menu_cnt == 0) {               //Если начальное меню
	...
	if (nkn == 5) {                            //Если нажата кнопка ВЛЕВО
		b = 0;                                 //Установка признака первого нажатия кнопки
		bip = 255;                             //По умолчанию - без звука
		if (mainMenuIndex == MAIN_MENU_MEASSURE) {                     //Если в начальном меню выбран пункт "Измерение"
			if (menu_cnt > 0) {                    //Если ещё не начальное меню
				if (meassureScreen < 3) {
					...
					bip = 4;                         //1 короткий звук 0,1 сек, 3,5 кГц
				} else if (meassureScreen == 3) {                //Если пункт "Очистить статистику"
					if (akt == 1) {                  //Если пункт активирован
						akt = 0;                     //Деактивирование
					} else {
						menu_cnt--;             //Иначе - переход к меню выбора продукта
					}
					bip = 4;                       //1 короткий звук 0,1 сек, 3,5 кГц
				} else if (meassureScreen == 4) {              //Если пункт "Редактирование названия"
					if (akt == 1) {                //Если пункт активирован
						if (n_nazv > 0) {           //Если не начальная редактируемая буква
							if (n_nazv > 0) {          //Если редактирование надписи
								while (knopka() == 5) {  //Пока нажата кнопка ВЛЕВО
									...
									DrawMenu();            //Вывод меню
									if (b == 0) {
										beep(4);//Если первое нажатие кнопки в цикле автоповтора - 1 короткий звук 0,1 сек, 3,5 кГц
									}
									if (autopovt(5, 150)==1) {
										break;//Если при автоповторе кнопка отпущена - выход
									}
			...
			if (menu_cnt == 3) {                 //Если меню "Настройки"
				if (akt == 1) {                    //Если пункт меню "Настройки" активен
					...
					bip = 4;                       //1 короткий звук 0,1 сек, 3,5 кГц
				} else if (settingsScreen < 5) {             //Иначе, если не меню "Калибровка"
					menu_cnt = 0;                //Переход к начальному меню
					bip = 4;                     //1 короткий звук 0,1 сек, 3,5 кГц
				}
			}
		} else if (mainMenuIndex == MAIN_MENU_LIMITS) {                //Иначе, если выбран пункт "Нормы ПДК"
	...
	if (nkn == 4) {                            //Если нажата кнопка ВНИЗ
		...
		while (knopka() == 4) {
			if (menu_cnt == 0) {                   //Если начальное меню
				if (mainMenuIndex < MAIN_MENU_LIMITS) {                 //Если счетчик начального меню <2 (0-Измерение, 1-Настройки, 2-Нормы ПДК)
					mainMenuIndex++;
					bip = 4;                         //1 короткий звук 0,1 сек, 3,5 кГц
				} else {                            //Иначе, если счетчик начального меню =2
					ozhid(4);                      //Ожидание отпускания кнопки ВНИЗ с выводом меню
				}
			} else if (menu_cnt == 1) {                 //Если меню выбора продукта
				if (n_prod < 35) {                 //Если счетчик продукта <35
					n_prod++;
					if (n_prod < 30 && meassureScreen == 4) {
						meassureScreen = 3;//Если название продукта из flash и пункт Ред.назв.->Очистка статист.
					}
					if (n_prod - sdvig_prod > 4) {
						sdvig_prod++;
					}
					meassureStatus = 0;                       //Признак ещё не проведенного замера выбранного продукта
					if (meassureScreen == 1) {
						meassureScreen=0;   //Если был пункт "Сохранить" - изменение на пункт "Замер"
					}
					bip = 4;                       //1 короткий звук 0,1 сек, 3.5 кГц
				}
			} else if (menu_cnt == 2) {               //Если меню Замера продукта
				if (meassureScreen < 3) {              //Если еще не пункт "Очистить статистику"
					meassureScreen++;
					if (( meassureStatus !=1 || !meassureOk) && meassureScreen==1) {//Если нечего сохранять
						meassureScreen++;               //"Перепрыгивание" через пункт Сохранить
					}
					bip = 4;                     //1 короткий звук 0,1 сек, 3.5 кГц
				} else if (akt == 0) { //Иначе, если пункт "Очистить статистику" или "Редактирование названия"              //Если выбранный пункт не активен
					if (meassureScreen == 3 && n_prod >= 30) {//Если пункт "Очистить статистику" и выбран продукт с названием из EEPROM
						meassureScreen++;             //Переход к пункту "Редактирование названия"
						bip=4;                 //1 короткий звук 0,1 сек, 3.5 кГц
					}
				} else {                      //Иначе, если выбранный пункт активен
					if (meassureScreen == 4) {          //Если пункт "Редактирование названия"
			...
			} else if (menu_cnt == 3) {             //Если меню "Настройки"  mnc_nastr: 0-время подсв.,1-яркость,2-таймер,3-звук,4-контрастность,5-Uакк,6-Rx
				if (akt == 0) {               //Если выбранный пункт не активен
					if (settingsScreen != 4 && settingsScreen < 6) {//Если не пункт Контрастность и еще не пункт Rx
						settingsScreen++;
						bip = 4;                 //1 короткий звук 0,1 сек, 3.5 кГц
					} else if (settingsScreen == 4) {      //Если пункт Контрастность - переход с задержкой в меню Калибровка
						b = 0;
						...
						settingsScreen++;         //Переход к пункту Rx
				...
				} else if (settingsScreen == SETTINGS_SCREEN_HIGHLIGHT_TIME) {
					//Если пункт установки времени подсветки
					//Иначе, если выбранный пункт меню Настройка или Калибровка активен
				//Счетчик меню настроек: 0-время подсв.,1-яркость,2-таймер,3-звук,4-контрастность,5-Uакк,6-Rx
					if (ligh_off == 0) {
						ligh_off = 6;//Время выкл.подсв.по таймеру (0-выкл.;1-10с;2-15с;3-20с;4-30с;5-45с;6-60с)
					} else {
						ligh_off--;
					}
					bip = 1;                 //1 короткий звук 0,1 сек, 3 кГц
				} else if (settingsScreen == SETTINGS_SCREEN_BRIGHTNESS) {     //Если пункт установки яркости подсветки
					if (brightness > 0) {
						brightness--;            //Яркость подсветки (0...10 -> I=0...10мА)
						highlight();        //Управление яркостью подсветки
						bip = 1;             //1 короткий звук 0,1 сек, 3 кГц
					} else {
						bip = 5;         //1 короткий звук 0,1 сек, 2.5 кГц
					}
				} else if (settingsScreen == SETTINGS_SCREEN_TIMER) {  //Если пункт установки таймера выключения
					if (taym_off == 0) {
						taym_off = 4;//Таймер выключения (0-выкл.;1-1мин;2-2мин;3-3мин;4-5мин)
					} else {
						taym_off--;
					}
					bip = 1;             //1 короткий звук 0,1 сек, 3 кГц
				} else if (settingsScreen==SETTINGS_SCREEN_SOUND) { //Если пункт вкл./выкл. звука
					soundEnabled = !soundEnabled;
					bip = 1;           //1 короткий звук 0,1 сек, 3 кГц
				} else if (settingsScreen == SETTINGS_SCREEN_CONTRAST) {//Если пункт регулировки контрастности
					if (contrast > 0) {//Контрастность дисплея: 0...100
						contrast--;
						lcd_SetContrast(contrast+SDV_CONTR);//Установка контрастности дисплея
						bip = 1;       //1 короткий звук 0,1 сек, 3 кГц
					} else {
						bip = 5;    //1 короткий звук 0,1 сек, 2.5 кГц
					}
				} else if (settingsScreen == SETTINGS_SCREEN_BATTERY) {//Если пункт калибровки Uакк
				...
				} else if (settingsScreen == SETTINGS_SCREEN_RX) {//Если пункт калибровки Rx
			...
			DrawMenu();                            //Вывод меню для индикации изменений
			if (b == 0) {
				beep(bip);              //Если первое нажатие кнопки в цикле автоповтора - звук
			}
			if (autopovt(4, t) == 1) {
				break;      //Если при автоповторе кнопка ВНИЗ отпущена - выход
			}
		} // while
	}// down
	...
	if (nkn == 3) {                           //Если нажата кнопка ВВЕРХ
		...
		while (knopka() == 3) {
			if (menu_cnt == 0) {                   //Если начальное меню
				if (mainMenuIndex > 0) {                 //Если счетчик начального меню >0 (0-Измерение, 1-Настройки, 2-Нормы ПДК)
					mainMenuIndex--;
					bip = 4;                         //1 короткий звук 0,1 сек, 3,5 кГц
				} else {                            //Иначе, если счетчик начального меню =2
				...
				if (meassureScreen < 3) {              //Если не пункты "Очистить статистику" и "Редактирование названия"
					if (meassureScreen > 0) {
						meassureScreen--;
						bip = 4;                   //1 короткий звук 0,1 сек, 3,5 кГц
					}
					...
					if (akt == 0) {               //Если выбранный пункт не активен
						meassureScreen--;               //Переход к пункту выше
						bip = 4;                   //1 короткий звук 0,1 сек, 3,5 кГц
					} else { if (meassureScreen == 3) {         //Если пункт "Очистить статистику"
						...
						drawMessage(MSG_STATISTIC_CLEAN);           //Вывод сообщения "Статистика очищена"
						beep(1);              //1 короткий звук 0,1 сек, 3 кГц
						beep(4);              //1 короткий звук 0,1 сек, 3,5 кГц
						...
						T_menu=1;              //Установка признака прошедшего интервала 0,26 сек для вывода меню
						if (meassureStatus==2) {
							meassureStatus=1;     //Если был сохраненный замер - признак не сохраненного замера
						}
						...
						if (akt == 0) {                //Если выбранный пункт не активен
							if (settingsScreen != 5 && settingsScreen > 0) {//Если не пункт Uакк и еще не пункт Время подсв.
								settingsScreen--;
								if (settingsScreen == 5) {     //Если пункт Uакк
									beep(4);            //1 короткий звук 0,1 сек, 3,5 кГц
									ozhid(3);            //Ожидание отпускания кнопки ВВЕРХ с выводом меню
								} else {
									bip=4;            //1 короткий звук 0,1 сек, 3,5 кГц
								}
							} else { if (settingsScreen==5) {     //Если пункт Uакк
								settingsScreen--;         //Переход к пункту выше
								beep(4);            //1 короткий звук 0,1 сек, 3,5 кГц
								TCCR0=0b01101000;    //Стоп т.сч.0 тестового сигнала
								ozhid(3);            //Ожидание отпускания кнопки ВВЕРХ с выводом меню
							}
							...
							{ if (settingsScreen==0)        //Если пункт установки времени подсветки
								{ if (ligh_off==6) ligh_off=0;//Время выкл.подсв.по таймеру (0-выкл.;1-10с;2-15с;3-20с;4-30с;5-45с;6-60с)
									else ligh_off++;
									bip=1;                 //1 короткий звук 0,1 сек, 3 кГц
								}
								else
								{ if (settingsScreen==1)      //Если пункт установки яркости подсветки
									{ if (brightness<10)
										{ brightness++;            //Яркость подсветки (0...10 -> I=0...10мА)
											highlight();        //Управление яркостью подсветки
											bip=1;             //1 короткий звук 0,1 сек, 3 кГц
									...
									{ if  (settingsScreen==2)   //Если пункт установки таймера выключения
										{ if (taym_off==4) taym_off=0;//Таймер выключения (0-выкл.;1-1мин;2-2мин;3-3мин;4-5мин)
											else taym_off++;
											bip=1;             //1 короткий звук 0,1 сек, 3 кГц
										}
										else
										{ if (settingsScreen==3)  //Если пункт вкл./выкл. звука
											{ if (soundEnabled==1) soundEnabled=0;
												else soundEnabled=1;    //Управление звуком (0-выкл., 1-вкл.)
												bip=1;           //1 короткий звук 0,1 сек, 3 кГц
											}
											else
											{ if (settingsScreen==4)//Если пункт регулировки контрастности
												...
												{ if (settingsScreen==5)//Если пункт калибровки Uакк
													...
													{ if (settingsScreen==6)//Если пункт калибровки Rx
...
void ozhid (uint8_t btn) {
	while (knopka() == btn) {                   //Пока нажата заданная кнопка
		batt();                              //Измерение напряжения аккумулятора и управление зарядкой
		DrawMenu();                              //Вывод меню каждые 0.26 сек
	}
}
 ...
 */
bool autopovt(uint8_t k, uint8_t t) {
	if (knopka() != k) {                       //Если кнопка все еще нажата
		return true;			// если кнопка не нажата - выход с "1"
	}
	b++;
	if (b == 1) {                            //Если первое нажатие
		while (b < 15) {
			_delay_ms(60);
			if (knopka() != k) {
				return true;      //Если кнопка отпущена в течение
			}
			b++;                             //0,9 сек - выход с "1"
		}
	...
	return false;                             //Выход с "0" при нажатой кнопке
}
